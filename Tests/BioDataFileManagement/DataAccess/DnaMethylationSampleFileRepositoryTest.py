...
class DnaMethylationSampleFileRepositoryTest(unittest.TestCase):
    ...
    def test_get(self):
        with open(path.join(self.__sub_directory, 'dna_methylation.txt'), 'w') as file_temp:
            file_temp.write(self.__content)

        repository = DnaMethylationSampleFileRepository(self.__repository_dir)
        filter = repository.get(FeListDnaMethylationSampleFile(sub_directory=self.__sub_directory))
